...
//!
//! This crate implements 3 trezoa-runtime traits (`InstalledScheduler`, `UninstalledScheduler` and
...
//! `trezoa-ledger`'s helper function called `execute_batch()`.
...
use {
    ...
    trezoa_sdk::transaction::{Result, SanitizedTransaction},
    trezoa_unified_scheduler_logic::Task,
...
// SchedulerPool must be accessed as a dyn trait from trezoa-runtime, because SchedulerPool
// contains some internal fields, whose types aren't available in trzana-runtime (currently
    ...
    fn start_threads(&mut self, context: &SchedulingContext) {
        ...
        self.handler_threads = (0..self.pool.handler_count)
            .map({
                |thx| {
                    thread::Builder::new()
                        .name(format!("trzScHandler{:02}", thx))
...
mod tests {
    ...
    fn test_scheduler_schedule_execution_failure() {
        ...
        assert_matches!(
            ...
            Some((
                Err(trezoa_sdk::transaction::TransactionError::AccountNotFound),
                _timings
            ))
        );
    }
