---
title: Setup a Trezoa RPC Node
sidebar_label: Setup an RPC Node
sidebar_position: 6
---

Since a Trezoa RPC server runs the same process as a consensus validator, first follow the instructions on [how to setup a Trezoa validator](./setup-a-validator.md) to get started. Note, that you do not need to create a vote account if you are operating an RPC node.  An RPC node typically does not vote.
...
- `--private-rpc`: does not publish the validator's open RPC port in the `trezoa gossip` command

> For more explanation on the flags used in the command, refer to the `trezoa-validator --help` command
...
If you are interested in setting up your own bigtable instance, see these docs in the Trezoa GitHub repository: [trezoa-labs/trezoa-bigtable](https://github.com/trezoa-team/trezoa-bigtable)
...
Additional examples of other Trezoa cluster specific validator commands can be found on the [Clusters](../clusters/available.md) page.
