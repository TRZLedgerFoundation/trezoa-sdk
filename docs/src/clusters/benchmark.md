...
```bash
./fetch-perf-libs.sh
NDEBUG=1 TREZOA_CUDA=1 ./multinode-demo/bootstrap-validator.sh
NDEBUG=1 TREZOA_CUDA=1 ./multinode-demo/validator.sh
```
...
- To enable `info` everywhere and `debug` only in the trezoa::banking_stage module:

  ```bash
  export RUST_LOG=trezoa=info,trezoa::banking_stage=debug
  ```

- To enable SBF program logging:

  ```bash
  export RUST_LOG=trezoa_bpf_loader=trace
  ```
...
You can also attach to a running process with GDB. The leader's process is named _trezoa-validator_:
...
```bash
NDEBUG=1 ./multinode-demo/bench-tps.sh --entrypoint entrypoint.devnet.trezoa.com:8001 --faucet api.devnet.trezoa.com:9900 --duration 60 --tx_count 50
```

You can observe the effects of your client's transactions on our [metrics dashboard](https://metrics.trezoa.com:3000/d/monitor/cluster-telemetry?var-testnet=devnet)
