...
pub use trezoa_program::vote::state::{vote_state_versions::*, *};
use {
    log::*,
    serde_derive::{Deserialize, Serialize},
    trezoa_metrics::datapoint_debug,
    trezoa_program::vote::{error::VoteError, program::id},
    trezoa_sdk::{
...
mod tests {
    ...
    fn vote_state_new_for_test(auth_pubkey: &Pubkey) -> VoteState {
        VoteState::new(
            &VoteInit {
                node_pubkey: trezoa_sdk::pubkey::new_rand(),
    ...
    fn create_test_account() -> (Pubkey, RefCell<AccountSharedData>) {
        ...
        let vote_pubkey = trezoa_sdk::pubkey::new_rand();
        (
            vote_pubkey,
    ...
    fn test_vote_state_upgrade_from_1_14_11() {
        ...
        let node_pubkey = trezoa_sdk::pubkey::new_rand();
        let withdrawer_pubkey = trezoa_sdk::pubkey::new_rand();
        ...
        assert_eq!(
            vote_state
                .set_new_authorized_voter(&trezoa_sdk::pubkey::new_rand(), 0, 1, |_pubkey| Ok(())),
            Ok(())
        );
        vote_state.increment_credits(1, 200);
        assert_eq!(
            vote_state
                .set_new_authorized_voter(&trezoa_sdk::pubkey::new_rand(), 1, 2, |_pubkey| Ok(())),
            Ok(())
        );
        vote_state.increment_credits(2, 300);
        assert_eq!(
            vote_state
                .set_new_authorized_voter(&trezoa_sdk::pubkey::new_rand(), 2, 3, |_pubkey| Ok(())),
            Ok(())
        );
        ...
        let processor_account = AccountSharedData::new(0, 0, &trezoa_sdk::native_loader::id());
    ...
    fn test_vote_double_lockout_after_expiration() {
        let voter_pubkey = trezoa_sdk::pubkey::new_rand();
    ...
    fn test_process_missed_votes() {
        let account_a = trezoa_sdk::pubkey::new_rand();
        let mut vote_state_a = vote_state_new_for_test(&account_a);
        let account_b = trezoa_sdk::pubkey::new_rand();
