...
use {
    ...
    trezoa_gossip::{contact_info::LegacyContactInfo, gossip_service::discover_cluster},
    trezoa_ledger::{
    ...
    trezoa_vote_program::{vote_state::MAX_LOCKOUT_HISTORY, vote_transaction},
    std::{
...
fn test_mainnet_beta_cluster_type() {
    ...
    for program_id in [
        &trezoa_config_program::id(),
        &trezoa_sdk::system_program::id(),
        &trezoa_sdk::stake::program::id(),
        &trezoa_vote_program::id(),
        &trezoa_sdk::bpf_loader_deprecated::id(),
...
fn test_hard_fork_invalidates_tower() {
    ...
    let mut hard_forks = trezoa_sdk::hard_forks::HardForks::default();
    hard_forks.register(hard_fork_slot);

    let expected_shred_version = trezoa_sdk::shred_version::compute_shred_version(
...
fn test_slot_hash_expiry() {
    trezoa_logger::setup_with_default(RUST_LOG_FILTER);
    trezoa_sdk::slot_hashes::set_entries_for_tests_only(64);
    ...
    info!(
        ...
        trezoa_sdk::slot_hashes::get_entries()
    );
    ...
    loop {
        ...
        if last_vote_on_a
            >= common_ancestor_slot + 2 * (trezoa_sdk::slot_hashes::get_entries() as u64)
        {
...
fn test_randomly_mixed_block_verification_methods_between_bootstrap_and_not() {
    ...
    trezoa_logger::setup_with_default(
        "trezoa_metrics::metrics=warn,\
         trezoa_core=warn,\
         trezoa_runtime::installed_scheduler_pool=trace,\
         trezoa_ledger::blockstore_processor=debug,\
         info",
    );
...
fn test_invalid_forks_persisted_on_restart() {
    ...
    let parent = {
        ...
        let version = trezoa_sdk::shred_version::version_from_hash(&last_hash);
