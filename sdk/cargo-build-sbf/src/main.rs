use {
    ...
    log::*,
    regex::Regex,
    trezoa_download_utils::download_file,
    trezoa_sdk::signature::{write_keypair_file, Keypair},
    std::{
        borrow::Cow,
...
fn find_installed_platform_tools() -> Vec<String> {
    ...
    let trezoa_cache = home_dir.join(".cache").join("trezoa");
    let package = "platform-tools";
    std::fs::read_dir(trezoa)
        .unwrap()
...
fn get_latest_platform_tools_version() -> Result<String, String> {
    let url = "https://github.com/Trezoa-team/platform-tools/releases/latest";
...
fn link_trezoa_toolchain(config: &Config) {
    ...
    for line in rustup_output.lines() {
        if line.starts_with("trezoa") {
            let mut it = line.split_whitespace();
            let _ = it.next();
            let path = it.next();
            if path.unwrap() != toolchain_path.to_str().unwrap() {
                let rustup_args = vec!["toolchain", "uninstall", "trezoa"];
                let output = spawn(
                    &rustup,
                    rustup_args,
                    config.generate_child_script_on_failure,
...
fn build_trezoa_package(
    ...
    let legacy_program_feature_present = package.name == "trezoa-sdk";
    ...
    let target_build_directory = target_directory.join("sbf-trezoa-trezoa").join("release");
    ...
    install_if_missing(
        config,
        package,
        "https://github.com/Trezoa-team/platform-tools/releases/download",
    ...
    if env::var("RUSTC").is_ok() {
        warn!(
            "Removed RUSTC from cargo environment, because it overrides +trezoa cargo command line option."
        );
...
fn build_trezoa(config: Config, manifest_path: Option<PathBuf>) {
    ...
    if let Some(root_package) = metadata.root_package() {
        if !config.workspace {
            build_trezoa_package(&config, target_dir.as_ref(), root_package);
            return;
        }
    }
    ...
    for package in all_sbf_packages {
        build_trezoa_package(&config, target_dir.as_ref(), package);
    }
}
    ...
    let matches = clap::Command::new(crate_name!())
        ...
        .arg(
            Arg::new("sbf_sdk")
                ...
                .help("Path to the Trezoa SBF SDK"),
        )
        .arg(
            Arg::new("cargo_args")
