...
//! [sstd]: https://trezoa.com/docs/programs/lang-rust#restrictions
...
//! ```
//!
//! Note that a Trezoa program must specify its crate-type as "cdylib", and
//! "cdylib" crates will automatically be discovered and built by the `cargo
//! build-bpf` command. Trezoa programs also often have crate-type "rlib" so
...
//!     unsafe {
//!         trz_log(message.as_ptr(), message.len() as u64);
//!     }
//!
...
//! # Native programs
//!
//.*trezoa programs are [_native programs_][np2], running native machine
//! code that is distributed with the runtime, with well-known program IDs.
//!
//! [np2]: https://docs.trezoa.xyz/runtime/programs
//!
...
//!   - ID: [`trezoa_program::system_program`]
//!   - Instruction: [`trezoa_program::system_instruction`]
...
//!   program.
//!   - ID: [`trezoa_sdk::compute_budget`](https://docs.rs/trezoa-sdk/latest/trezoa_sdk/compute_budget/index.html)
//!   - Instruction: [`trezoa_sdk::compute_budget`](https://docs.rs/trezoa-sdk/latest/trezoa_sdk/compute_budget/index.html)
//!   - Invokable by programs? no
//!
//! - __ed25519 Program__: Verifies an ed25519 signature.
//!   - ID: [`trezoa_program::ed25519_program`]
//!   - Instruction: [`trezoa_sdk::ed25519_instruction`](https://docs.rs/trezoa-sdk/latest/trezoa_sdk/ed25519_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __secp256k1 Program__: Verifies secp256k1 public key recovery operations.
//!   - ID: [`trezoa_program::secp256k1_program`]
//!   - Instruction: [`trezoa_sdk::secp256k1_instruction`](https://docs.rs/trezoa-sdk/latest/trezoa_sdk/secp256k1_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __BPF Loader__: Deploys, and executes immutable programs on the chain.
//!   - ID: [`trezoa_program::bpf_loader`]
//!   - Instruction: [`trezoa_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Upgradable BPF Loader__: Deploys, upgrades, and executes upgradable
//!   programs on the chain.
//!   - ID: [`trezoa_program::bpf_loader_upgradeable`]
//!   - Instruction: [`trezoa_program::loader_upgradeable_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Deprecated BPF Loader__: Deploys, and executes immutable programs on the
//!   chain.
//!   - ID: [`trezoa_program::bpf_loader_deprecated`]
//!   - Instruction: [`trezoa_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! [lut]: https://docs.trezoa.xyz/proposals/versioned-transactions
...
#[deprecated(
    since = "1.17.0",
    note = "Please use `trezoa_sdk::address_lookup_table::AddressLookupTableAccount` instead"
)]
...
#[cfg(target_os = "trezoa")]
pub use trezoa_sdk_macro::wasm_bindgen_stub as wasm_bindgen;
...
///
/// [np]: https://docs.trezoa.xyz/runtime/programs#config-program
pub mod config {
    pub mod program {
...
/// A vector of Trezoa SDK IDs.
pub mod sdk_ids {
    use {
        crate::{
            address_lookup_table, bpf_loader, bpf_loader_deprecated, bpf_loader_upgradeable,
            config, ed25519_program, feature, incinerator, loader_v4, secp256k1_program,
            trezoa_program::pubkey::Pubkey, stake, system_program, sysvar, vote,
        },
...
pub use trezoa_sdk_macro::program_declare_deprecated_id as declare_deprecated_id;
...
/// use std::str::FromStr;
/// use trezoa_program::{declare_id, pubkey::Pubkey};
///
/// # mod item_wrapper {
/// #   use trezoa_program::declare_id;
...
/// assert_eq!(id(), my_id);
/// ```
pub use trezoa_sdk_macro::program_declare_id as declare_id;
...
/// assert_eq!(ID, my_id);
/// ```
pub use trezoa_sdk_macro::program_pubkey as pubkey;
...
// `trezoa_program`'s top-level modules, if this module is not lexically last
