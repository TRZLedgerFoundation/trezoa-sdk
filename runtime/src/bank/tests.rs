...
use {
    ...
    trezoa_stake_program::stake_state::{self, StakeStateV2},
    trezoa_svm::transaction_results::DurableNonceFee,
    trezoa_vote_program::{
        vote_instruction,
        vote_state::{
...
fn create_genesis_config_no_tx_fee(lamports: u64) -> (GenesisConfig, Keypair) {
    ...
    let (mut genesis_config, mint_keypair) =
        trezoa_sdk::genesis_config::create_genesis_config(lamports);
...
fn test_race_register_tick_freeze() {
    ...
    let p = trezoa_sdk::pubkey::new_rand();
...
fn test_rent_distribution() {
    ...
    let validator_1_pubkey = trezoa_sdk::pubkey::new_rand();
    ...
    let validator_2_pubkey = trezoa_sdk::pubkey::new_rand();
    ...
    let validator_3_pubkey = trezoa_sdk::pubkey::new_rand();
...
fn test_rent_exempt_executable_account() {
    ...
    let mut account = AccountSharedData::new(account_balance, 0, &trezoa_sdk::pubkey::new_rand());
...
fn test_rent_eager_collect_rent_in_partition(should_collect_rent: bool) {
    ...
    for feature_id in FeatureSet::default().inactive {
        if feature_id != trezoa_sdk::feature_set::skip_rent_rewrites::id()
            && (!should_collect_rent
                || feature_id != trezoa_sdk::feature_set::disable_rent_fees_collection::id())
        {
            activate_feature(&mut genesis_config, feature_id);
        }
    }

    let zero_lamport_pubkey = trezoa_sdk::pubkey::new_rand();
    let rent_due_pubkey = trezoa_sdk::pubkey::new_rand();
    let rent_exempt_pubkey = trezoa_sdk::pubkey::new_rand();
    let mut bank = Arc::new(Bank::new_for_tests(&genesis_config));
...
fn do_test_bank_update_rewards_determinism() -> u64 {
    ...
    let vote_id = trezoa_sdk::pubkey::new_rand();
    let mut vote_account =
        vote_state::create_account(&vote_id, &trezoa_sdk::pubkey::new_rand(), 0, 100);
    let stake_id1 = trezoa_sdk::pubkey::new_rand();
    let stake_account1 = crate::stakes::tests::create_stake_account(123, &vote_id, &stake_id1);
    let stake_id2 = trezoa_sdk::pubkey::new_rand();
...
fn test_transfer_to_sysvar() {
    ...
    let normal_pubkey = trezoa_sdk::pubkey::new_rand();
...
fn test_bank_tx_fee() {
    ...
    let leader = trezoa_sdk::pubkey::new_rand();
    let GenesisConfigInfo {
        mut genesis_config,
        mint_keypair,
        ..
...
fn test_readonly_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &trezoa_sdk::pubkey::new_rand(), 0);
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;

    let vote_pubkey0 = trezoa_sdk::pubkey::new_rand();
    let vote_pubkey1 = trezoa_sdk::pubkey::new_rand();
    let vote_pubkey2 = trezoa_sdk::pubkey::new_rand();
...
fn test_readonly_relaxed_locks() {
    ...
    let key3 = trezoa_sdk::pubkey::new_rand();
...
fn test_bank_update_sysvar_account() {
    ...
    for pass in 0..5 {
        ...
        let dummy_clock_id = trezoa_sdk::pubkey::new_rand();
...
fn test_bank_vote_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &trezoa_sdk::pubkey::new_rand(), 1);
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;
...
fn test_bank_cloned_stake_delegations() {
    ...
    let (vote_balance, stake_balance) = {
        ...
        let minimum_delegation = trezoa_stake_program::get_minimum_delegation(&bank.feature_set);
        (
...
fn test_is_delta_with_no_committables() {
    ...
    assert_eq!(
        bank.transfer(10_001, &mint_keypair, &trezoa_sdk::pubkey::new_rand()),
        Err(TransactionError::InstructionError(
            0,
            SystemError::ResultWithNegativeLamports.into(),
        ))
...
fn test_bank_get_program_accounts() {
    ...
    assert!(
        genesis_accounts
            .iter()
            .any(|(pubkey, _, _)| trezoa_sdk::sysvar::is_sysvar_id(pubkey)),
        "no sysvars found"
    );
...
fn test_account_ids_after_program_ids() {
    ...
    tx.message.account_keys.push(trezoa_sdk::pubkey::new_rand());
...
fn test_bank_hash_consistency() {
    ...
    activate_feature(
        &mut genesis_config,
        trezoa_sdk::feature_set::set_exempt_rent_epoch_max::id(),
    );
...
fn test_same_program_id_uses_unique_executable_accounts() {
    ...
    let program1_pubkey = trezoa_sdk::pubkey::new_rand();
    let bank =
        Bank::new_with_mockup_builtin_for_tests(&genesis_config, program1_pubkey, MockBuiltin::vm)
            .0;

    // Add a new program owned by the first
    let program2_pubkey = trezoa_sdk::pubkey::new_rand();
...
fn test_add_builtin_loader_no_overwrite() {
    let slot = 123;
    let loader_id = trezoa_sdk::pubkey::new_rand();
...
fn test_reconfigure_token2_native_mint() {
    ...
    let genesis_config =
        create_genesis_config_with_leader(5, &trezoa_sdk::pubkey::new_rand(), 0).genesis_config;
    let bank = Arc::new(Bank::new_for_tests(&genesis_config));
    assert_eq!(
        bank.get_balance(&inline_tpl_token::native_mint::id()),
        1000000000
    );
    let native_mint_account = bank
        .get_account(&inline_tpl_token::native_mint::id())
        .unwrap();
    assert_eq!(native_mint_account.data().len(), 82);
    assert_eq!(native_mint_account.owner(), &inline_tpl_token::id());
}
...
fn test_bank_load_program() {
    ...
    let programdata_key = trezoa_sdk::pubkey::new_rand();
        ...
        ) + std::marker::Send,
{
    ...
    // Set up initial bank
    let mut genesis_config =
        create_genesis_config_with_leader(10, &trezoa_sdk::pubkey::new_rand(), 374_999_998_287_840)
            .genesis_config;
    ...
    let all_pubkeys: Vec<Pubkey> = std::iter::repeat_with(trezoa_sdk::pubkey::new_rand)
        .take(total_pubkeys)
        .collect();
...
fn test_get_inflation_start_slot_devnet_testnet() {
    let GenesisConfigInfo {
        mut genesis_config, ..
    } = create_genesis_config_with_leader(42, &trezoa_sdk::pubkey::new_rand(), 42);
    genesis_config
        .accounts
...
fn test_call_precomiled_program() {
    ...
    let instruction =
        trezoa_sdk::secp256k1_instruction::new_secp256k1_instruction(&secp_privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    let message_arr = b"hello";
    let instruction =
        trezoa_sdk::ed25519_instruction::new_ed25519_instruction(&privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
...
fn test_an_empty_instruction_without_program() {
    ...
    let destination = trezoa_sdk::pubkey::new_rand();
...
fn test_invalid_rent_state_changes_fee_payer() {
    ...
    genesis_config.fee_rate_governor = FeeRateGovernor::new(
        ...
        trezoa_sdk::fee_calculator::DEFAULT_TARGET_SIGNATURES_PER_SLOT,
    );
...
fn test_rent_state_incinerator() {
    ...
    for amount in [rent_exempt_minimum - 1, rent_exempt_minimum] {
        bank.transfer(amount, &mint_keypair, &trezoa_sdk::incinerator::id())
            .unwrap();
    }
}
...
fn test_resize_and_rent() {
    ...
    let rent_paying_pubkey = trezoa_sdk::pubkey::new_rand();
...
fn test_accounts_data_size_and_rent_collection(should_collect_rent: bool) {
    ...
    if should_collect_rent {
        genesis_config
            .accounts
            .remove(&trezoa_sdk::feature_set::disable_rent_fees_collection::id());
    }
...
fn test_cap_accounts_data_allocations_per_transaction() {
    ...
    assert_eq!(
        ...
        Err(TransactionError::InstructionError(
            ...
            trezoa_sdk::instruction::InstructionError::MaxAccountsDataAllocationsExceeded,
        )),
    );
}
...
fn test_is_in_slot_hashes_history() {
    use trezoa_sdk::slot_hashes::MAX_ENTRIES;
...
fn test_program_execution_restricted_for_stake_account_in_reward_period() {
    use trezoa_sdk::transaction::TransactionError::ProgramExecutionTemporarilyRestricted;
