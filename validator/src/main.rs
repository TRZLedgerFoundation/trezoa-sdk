...
pub fn main() {
    ...
    if !matches.is_present("no_os_network_limits_test") {
        if SystemMonitorService::check_os_network_limits() {
            ...
            eprintln!("OS network limit test failed. See: https://docs.trezoa.xyz/operations/guides/validator-start#system-tuning");
            exit(1);
        }
    }
...
fn process_account_indexes(matches: &ArgMatches) -> AccountSecondaryIndexes {
    let account_indexes: HashSet<AccountIndex> = matches
        ...
        .map(|value| match value {
            ...
            "tpl-token-mint" => AccountIndex::TplTokenMint,
            "tpl-token-owner" => AccountIndex::TplTokenOwner,
            _ => unreachable!(),
        })
        .collect();
