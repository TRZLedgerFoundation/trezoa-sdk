...
ok=true
for program in trezoa-{faucet,genesis,keygen,validator}; do
  $program -V || ok=false
done
$ok || {
  echo
...
export RUST_LOG=${RUST_LOG:-trezoa=info,trezoa_runtime::message_processor=debug} # if RUST_LOG is unset, default to info
...
if [[ -e "$ledgerDir"/genesis.bin || -e "$ledgerDir"/genesis.tar.bz2 ]]; then
  ...
  if [[ -r tpl-genesis-args.sh ]]; then
    TPL_GENESIS_ARGS=$(cat tpl-genesis-args.sh)
  fi
