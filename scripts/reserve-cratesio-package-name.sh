...
FLAGS:
        ...
        --no-trezoa-prefix    Do not require \`trezoa-\` prefix on PACKAGE_NAME
...
if ${require_trezoa_prefix} && ! [[ "${package_name}" =~ ^trezoa- ]]; then
  # shellcheck disable=SC2016 # backticks are not a command here
  echo 'error: PACKAGE_NAME MUST start with `trezoa-`' 1>&2
  display_help
  exit 1
fi
