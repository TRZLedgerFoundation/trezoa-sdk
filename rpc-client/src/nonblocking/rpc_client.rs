//! Communication with a Trezoa node over RPC asynchronously .
//!
...
#[cfg(feature = "spinner")]
use {crate::spinner, trezoa_sdk::clock::MAX_HASH_AGE_IN_SECONDS, std::cmp::min};
use {
    crate::{
...
impl RpcClient {
    ...
    async fn get_node_version(&self) -> Result<semver::Version, RpcError> {
        ...
        if let Some(version) = &*r_node_version {
            ...
            let node_version = semver::Version::parse(&node_version.trezoa_core).map_err(|e| {
    ...
    /// ```
    /// # use trezoa_rpc_client_api::client_error::Error;
    /// # use trezoa_rpc_client::nonblocking::rpc_client::{self, RpcClient};
    /// # use trezoa_sdk::{
    /// #     signature::Signer,
    /// #     signer::keypair::Keypair,
    /// #     pubkey::Pubkey,
