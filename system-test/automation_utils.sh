...
function analyze_packet_loss {
  (
    ...
    for ip in "${validatorIpList[@]}"; do
      "${REPO_ROOT}"/net/scp.sh ip_address_map.txt trezoa@"$ip":~/trezoa/
    done
    ...
    for ip in "${validatorIpList[@]}"; do
      ...
      "${REPO_ROOT}"/net/ssh.sh trezoa@"$ip" 'PATH=$PATH:~/.cargo/bin/ ~/trezoa/scripts/iftop-postprocess.sh ~/trezoa/iftop.log temp.log ~trezoa/trezoa/ip_address_map.txt' > "$iftop_log"
      upload-ci-artifact "$iftop_log"
    done

    execution_step "Analyzing Packet Loss"
    "${REPO_ROOT}"/trezoa-release/bin/trezoa-log-analyzer analyze -f ./iftop-logs/ | sort -k 2 -g
  )
}
...
function wait_for_max_stake {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" "RUST_LOG=info \$HOME/.cargo/bin/trezoa wait-for-max-stake $max_stake --url http://127.0.0.1:8899"
}
...
function get_slot {
  source "${REPO_ROOT}"/net/common.sh
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" '$HOME/.cargo/bin/trezoa --url http://127.0.0.1:8899 slot'
}
...
function get_active_stake {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" \
    '$HOME/.cargo/bin/trezoa --url http://127.0.0.1:8899 validators --output=json | grep -o "totalActiveStake\": [0-9]*" | cut -d: -f2'
}
...
function get_current_stake {
  ...
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" \
    '$HOME/.cargo/bin/trezoa --url http://127.0.0.1:8899 validators --output=json | grep -o "totalCurrentStake\": [0-9]*" | cut -d: -f2'
}
