...
if [[ -z $TREZOA_METRICS_CONFIG ]]; then
  if [[ -z $TREZOA_METRICS_PARTIAL_CONFIG ]]; then
    echo TREZOA_METRICS_PARTIAL_CONFIG not defined
    exit 1
  fi
  export TREZOA_METRICS_CONFIG="db=$TESTNET_TAG,host=$INFLUX_HOST,$TREZOA_METRICS_PARTIAL_CONFIG"
fi
echo "TREZOA_METRICS_CONFIG: $TREZOA_METRICS_CONFIG"
