 ...
 * Copies memory
 */
static void *trz_memcpy(void *dst, const void *src, int len) {
  for (int i = 0; i < len; i++) {
 ...
 * Compares memory
 */
static int trz_memcmp(const void *s1, const void *s2, int n) {
  for (int i = 0; i < n; i++) {
 ...
 */
static void *trz_memset(void *b, int c, size_t len) {
  uint8_t *a = (uint8_t *) b;
  while (len > 0) {
    *a = c;
    a++;
 ...
 */
static size_t trz_strlen(const char *s) {
  size_t len = 0;
  while (*s) {
    len++;
    s++;
 ...
 */
static void *trz_calloc(size_t nitems, size_t size) {
  // Bump allocator
...
/**
 * Deallocates the memory previously allocated by trz_calloc
 */
static void trz_free(void *ptr) {
