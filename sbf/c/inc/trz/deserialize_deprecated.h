#pragma once
/**
 * @brief Trezoa deprecated SBF loader deserializer to be used when deploying
 ...
 * instead it populates the pointers and lengths in TrzAccountInfo and data so
 ...
 * @param params Pointer to a TrzParameters structure
 * @return Boolean true if successful.
 */
static bool trz_deserialize_deprecated(
  const uint8_t *input,
  TrzParameters *params,
  uint64_t ka_num
) {
  ...
  for (int i = 0; i < params->ka_num; i++) {
    ...
    if (dup_info == UINT8_MAX) {
      ...
      // key
      params->ka[i].key = (TrzPubkey *) input;
      input += sizeof(TrzPubkey);
      ...
      // owner
      params->ka[i].owner = (TrzPubkey *) input;
      input += sizeof(TrzPubkey);
  ...
  params->program_id = (TrzPubkey *) input;
  input += sizeof(TrzPubkey);
