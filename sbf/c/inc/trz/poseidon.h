...
#ifndef TRZ_SBFV2
uint64_t trz_poseidon(
  const uint64_t parameters,
  const uint64_t endianness,
  const TrzBytes *bytes,
  const uint64_t bytes_len,
  uint8_t *result
);
#else
typedef uint64_t(*trz_poseidon_pointer_type)(
  const uint64_t parameters,
  const uint64_t endianness,
  const TrzBytes *bytes,
  const uint64_t bytes_len,
...
static uint64_t trz_poseidon(
  const uint64_t parameters arg1,
  const uint64_t endianness arg2,
  const TrzBytes *bytes arg3,
  const uint64_t bytes_len arg4,
  uint8_t *result
 arg5) {
  trz_poseidon_pointer_type trz_poseidon_pointer = (trz_poseidon_pointer_type) 3298065441;
  return trz_poseidon_pointer(arg1, arg2, arg3, arg4, arg5);
}
#endif
