 ...
 */
typedef struct {
  TrzPubkey *key;      /** Public key of the account */
  ...
  TrzPubkey *owner;    /** Program that owns this account */
 ...
 */
typedef struct {
  TrzAccountInfo* ka; /** Pointer to an array of TrzAccountInfo, must already
                          point to an array of TrzAccountInfos */
  uint64_t ka_num; /** Number of TrzAccountInfo entries in `ka` */
  const uint8_t *data; /** pointer to the instruction data */
  uint64_t data_len; /** Length in bytes of the instruction data */
  const TrzPubkey *program_id; /** program_id of the currently executing program */
} TrzParameters;

/**
 * Program instruction entrypoint
 *
 * @param input Buffer of serialized input parameters.  Use trz_deserialize() to decode
