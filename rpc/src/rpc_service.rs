...
use {
    ...
    trezoa_send_transaction_service::send_transaction_service::{self, SendTransactionService},
    ...
    match path {
        "/v0/circulating-supply" => {
            ...
            let non_circulating_supply =
                trezoa_runtime::non_circulating_supply::calculate_non_circulating_supply(&bank)
                    .expect("Scan should not error on root banks")
                    .lamports;
            Some(format!(
                "{}",
                lamports_to_trz(total_supply - non_circulating_supply)
            ))
        }
        "/v0/total-supply" => {
            let bank = bank_forks.read().unwrap().root_bank();
            let total_supply = bank.capitalization();
            Some(format!("{}", lamports_to_trz(total_supply)))
        }
        _ => None,
    }
}
...
mod tests {
    ...
    fn test_rpc_new() {
        ...
        let rpc_addr = SocketAddr::new(
            ip_addr,
            trezoa_net_utils::find_available_port_in_range(ip_addr, (10000, 65535)).unwrap(),
        );
