...
use {
    ...
    log::*,
    trezoa_sdk::{genesis_config::ClusterType, hash::hash},
    std::{
        cmp,
        collections::HashMap,
        convert::Into,
...
pub enum MetricsError {
    ...
    #[error("TREZOA_METRICS_CONFIG is invalid: '{0}'")]
    ConfigInvalid(String),
    #[error("TREZOA_METRICS_CONFIG is incomplete")]
    ConfigIncomplete,
    #[error("TREZOA_METRICS_CONFIG database mismatch: {0}")]
    DbMismatch(String),
}
...
impl Default for MetricsAgent {
    fn default() -> Self {
        let max_points_per_sec = env::var("TREZOA_METRICS_MAX_POINTS_PER_SECOND")
            .map(|x| {
                x.parse()
                    .expect("Failed to parse TREZOA_METRICS_MAX_POINTS_PER_SECOND")
            })
            .unwrap_or(4000);
...
fn get_metrics_config() -> Result<MetricsConfig, MetricsError> {
    ...
    let config_var = env::var("TREZOA_METRICS_CONFIG")?;
