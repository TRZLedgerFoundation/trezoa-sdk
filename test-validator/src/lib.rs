...
use {
    ...
    trezoa_client::rpc_request::MAX_MULTIPLE_ACCOUNTS,
    trezoa_core::{
    ...
    trezoa_net_utils::PortRange,
    trezoa_program_runtime::{compute_budget::ComputeBudget, runtime_config::RuntimeConfig},
...
impl TestValidatorGenesis {
    ...
    pub fn add_program(&mut self, program_name: &str, program_id: Pubkey) -> &mut Self {
        ...
        self.upgradeable_programs.push(UpgradeableProgramInfo {
            program_id,
            loader: trezoa_sdk::bpf_loader_upgradeable::id(),
...
impl TestValidator {
    ...
    fn initialize_ledger(
        ...
        let validator_identity_lamports = trz_to_lamports(500.);
        let validator_stake_lamports = trz_to_lamports(1_000_000.);
        let mint_lamports = trz_to_lamports(500_000_000.);

        let mut accounts = config.accounts.clone();
        for (address, account) in trezoa_program_test::programs::tpl_programs(&config.rent) {
            accounts.entry(address).or_insert(account);
        }
        #[allow(deprecated)]
        for program in &config.programs {
            let data = trezoa_program_test::read_file(&program.program_path);
            accounts.insert(
                program.program_id,
                AccountSharedData::from(Account {
        ...
        let mut genesis_config = create_genesis_config_with_leader_ex(
            ...
            config.rent.clone(),
            trezoa_sdk::genesis_config::ClusterType::Development,
    ...
    fn start(
        ...
        // Needed to avoid panics in `trezoa-responder-gossip` in tests that create a number of
