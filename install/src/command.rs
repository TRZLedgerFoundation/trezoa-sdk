use {
    ...
    trezoa_config_program::{config_instruction, get_config_data, ConfigState},
...
fn load_release_target(release_dir: &Path) -> Result<String, String> {
    ...
    version_yml.push("trezoa-release");
...
pub fn init(
    ...
    let config = {
        // Write new config file only if different, so that running |trezoa-install init|
...
fn github_release_download_url(release_semver: &str) -> String {
    format!(
        "https://github.com/trezoa-team/trezoa/releases/download/v{}/trezoa-release-{}.tar.bz2",
        release_semver,
...
fn release_channel_download_url(release_channel: &str) -> String {
    format!(
        "https://release.trezoa.com/{}/trezoa-release-{}.tar.bz2",
        release_channel,
        crate::build_env::TARGET
    )
}
...
fn release_channel_version_url(release_channel: &str) -> String {
    format!(
        "https://release.trezoa.com/{}/trezoa-release-{}.yml",
        release_channel,
        crate::build_env::TARGET
    )
}
...
pub fn info(config_file: &str, local_info_only: bool, eval: bool) -> Result<(), String> {
    ...
    if eval {
        ...
        config
            ...
            .and_then(|channel| {
                println!("TREZOA_INSTALL_ACTIVE_CHANNEL={channel}",);
                Option::<String>::None
            });
        return Ok(());
    }
...
fn check_for_newer_github_release(
    ...
    while page == 1 || releases.len() == PER_PAGE {
        let url = reqwest::Url::parse_with_params(
            "https://api.github.com/repos/trezoa-team/trezoa/releases",
            &[
...
pub fn init_or_update(config_file: &str, is_init: bool, check_only: bool) -> Result<bool, String> {
        ...
        match explicit_release {
            ...
            ExplicitRelease::Channel(release_channel) => {
                ...
                let current_release_version_yml =
                    release_dir.join("trezoa-release").join("version.yml");
    ...
    symlink_dir(
        release_dir.join("trezoa-release"),
