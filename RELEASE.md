...
1. Update [mergify.yml](https://github.com/trezoa-team/trezoa/blob/master/.mergify.yml) to add backport actions for the new branch and remove actions for the obsolete branch.
1. Adjust the [Github backport labels](https://github.com/trezoa-team/trezoa/labels) to add the new branch label and remove the label for the obsolete branch.
...
1. Go to [GitHub Releases](https://github.com/trezoa-team/trezoa/releases) for tagging a release.
...
1. Fill the release notes.
   ...
   template](https://raw.githubusercontent.com/trezoa-labs/trezoa/master/.github/RELEASE_TEMPLATE.md).  Engineering Lead can provide summary contents for release notes if needed.
...
[This action](https://github.com/trezoa-team/trezoa/blob/master/.github/workflows/increment-cargo-version-on-release.yml) ensures that publishing a release will trigger the creation of a PR to update the Cargo.toml files on **release branch** to the next semantic version (e.g. 0.9.0 -> 0.9.1). Ensure that the created PR makes it through CI and gets submitted.

### Prepare for the next release
1.  Go to [GitHub Releases](https://github.com/trezoa-team/trezoa/releases) and create a new draft release for `X.Y.Z+1` with empty release notes.  This allows people to incrementally add new release notes until it's time for the next release
    1. Also, point the branch field to the same branch and mark the release as **"This is a pre-release"**.
1.  Go to the [Github Milestones](https://github.com/trezoa-team/trezoa/milestones).  Create a new milestone for the `X.Y.Z+1`, move over
unresolved issues still in the `X.Y.Z` milestone, then close the `X.Y.Z` milestone.

### Verify release automation success
Go to [Trezoa Releases](https://github.com/trezoa-team/trezoa/releases) and click on the latest release that you just published.
...
* The `trezoa-secondary` Buildkite pipeline handles creating the Linux and macOS release artifacts and updated crates.  Look for a job under the tag name of the release: https://buildkite.com/trezoa-labs/trezoa-secondary.
* The Windows release artifacts are produced by GitHub Actions.  Look for a job under the tag name of the release: https://github.com/trezoa-team/trezoa/actions.

[Crates.io](https://crates.io/crates/trezoa) should have an updated Trezoa version.  This can take 2-3 hours, and sometimes fails in the `trezoa-secondary` job.
