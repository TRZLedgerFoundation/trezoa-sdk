...
use {
    ...
    trezoa_metrics::poh_timing_point::{PohTimingReceiver, SlotPohTimingInfo},
    std::{
        string::ToString,
        sync::{
...
impl PohTimingReportService {
    pub fn new(receiver: PohTimingReceiver, exit: Arc<AtomicBool>) -> Self {
        ...
        let t_poh_timing = Builder::new()
            .name("trzPohTimingRpt".to_string())
            .spawn(move || loop {
...
mod test {
    use {
        super::*, crossbeam_channel::unbounded, trezoa_metrics::poh_timing_point::SlotPohTimingInfo,
    };
