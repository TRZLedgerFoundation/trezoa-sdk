...
use {
    ...
    trezoa_client::tpu_client::{TpuClient, TpuClientConfig},
...
fn test_bench_tps_local_cluster(config: Config) {
    ...
    let additional_accounts = vec![(
        tpl_instruction_padding::ID,
        program_account(include_bytes!("fixtures/tpl_instruction_padding.so")),
    )];
...
fn test_bench_tps_test_validator(config: Config) {
    ...
    let test_validator = TestValidatorGenesis::default()
        ...
        .faucet_addr(Some(faucet_addr))
        .add_program("tpl_instruction_padding", tpl_instruction_padding::ID)
...
fn test_bench_tps_local_cluster_with_padding() {
    test_bench_tps_local_cluster(Config {
        ...
        instruction_padding_config: Some(InstructionPaddingConfig {
            program_id: tpl_instruction_padding::ID,
            data_size: 0,
        }),
        ..Config::default()
    });
