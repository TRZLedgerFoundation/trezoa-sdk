...
fn main() {
    ...
    if let Some(instruction_padding_config) = instruction_padding_config {
        ...
        client
            ...
            .expect("Instruction padding program must be deployed to this cluster. Deploy the program using `trezoa program deploy ./bench-tps/tests/fixtures/tpl_instruction_padding.so` and pass the resulting program id with `--instruction-padding-program-id`");
    }
